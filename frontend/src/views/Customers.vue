      ...
      <div style="padding:12px 16px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap;">
          ...
          <div style="display:flex;gap:6px;font-size:9px;font-weight:700;">
            <span style="color:#64748b;">{{ filtered.length }} customers</span>
          </div>
        </div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:11px;">
            <thead>
              <tr style="background:#f8fafc;">
                ...
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('customer_name')">Customer Name <span v-if="sortBy==='customer_name'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('customer_type')">Type <span v-if="sortBy==='customer_type'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('territory')">Territory <span v-if="sortBy==='territory'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('customer_group')">Group <span v-if="sortBy==='customer_group'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
              </tr>
            </thead>
            <tbody>
