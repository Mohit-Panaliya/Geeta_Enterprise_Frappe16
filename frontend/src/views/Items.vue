          ...
          <table style="width:100%;border-collapse:collapse;font-size:11px;">
            <thead>
              <tr style="background:#f8fafc;">
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('name')">Item Code <span v-if="sortBy==='name'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('item_name')">Item Name <span v-if="sortBy==='item_name'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
                <th style="text-align:left;padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;color:#64748b;cursor:pointer;" @click="sort('stock_uom')">UOM <span v-if="sortBy==='stock_uom'">{{ sortDir==='asc'?'\u25B2':'\u25BC' }}</span></th>
              </tr>
            </thead>
            <tbody>
